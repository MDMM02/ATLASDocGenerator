        ...
        private void RemoveTargetConditions(XDocument document)
        {
                ...
                .Concat(document.Root.Descendants()))
            {
                foreach (XAttribute attribute in element.Attributes()
                    .Where(attribute =>
                        attribute.Name.LocalName.Equals("conditions", StringComparison.OrdinalIgnoreCase)
                        || attribute.Name.LocalName.Equals("ConditionTagExpression", StringComparison.OrdinalIgnoreCase))
                    .ToList())
                {
                    attribute.Remove();
                }
            ...
            foreach (XElement expression in document.Root.Descendants()
